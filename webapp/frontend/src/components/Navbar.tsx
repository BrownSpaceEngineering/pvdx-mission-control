    ...
    return (
        <nav className="w-full bg-[#2F3F60] px-8 py-4 flex items-center justify-between relative">
            ...
            className={`fixed inset-0 z-40 bg-black transition-opacity duration-300 
                        ${showPvdxPanel ? "opacity-25 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
            />

            {/* Sliding PANEL (Right Side Drawer) */}
            <div
            className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl p-6 z-50 transform transition-transform transition-opacity duration-300
                ${showPvdxPanel ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"} 
                rounded-l-lg`} // Only left side corners rounded
            >
            {/* Close Button */}
            <button
                onClick={() => setShowPvdxPanel(false)}
